...
async function createInfo(info) {
    ...
    try {
        ...
        if (!deviceCol) {
            ...
            validationResponse.message = 'Game info entered.';
            validationResponse.responseCode = 200;
        } else {
            log.debug('Update: crudGame.createInfo : duplicate info, throwing error ');
            validationResponse.message = 'Duplicate game info entry.';
            validationResponse.responseCode = 455;
        }
    } catch (err) {
        log.error(`Update: crudGame.createInfo : Info insert failed : ${err}`);
        validationResponse.message = 'Info insert failed.';
