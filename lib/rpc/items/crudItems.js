...
async function createProperties(properties) {
    log.debug('Starting: crudItems.createProperties. properties object ');
    log.trace(`Starting: crudItems.createProperties. properties object : ${JSON.stringify(properties)} `);
    ...
    try {
        ...
        log.debug('Update: crudItems.createProperties. Is properties currently in DB? ');
        const deviceCol = await db.findOne(query);

        if (!deviceCol) {
            log.debug('Update: crudItems.createProperties : Properties not found in DB.   Adding Properties');
            ...
            log.debug('Update: crudItems.createProperties : duplicate properties, throwing error ');
            validationResponse.message = 'Duplicate item found!';
            validationResponse.responseCode = 455;
        }
    } catch (err) {
        log.error(`Update: crudItems.createProperties : Item insert failed. : ${err}`);
        validationResponse.message = 'Item insert failed.';
        validationResponse.responseCode = 457;
    }

    // Return Results
    log.debug(`Ending: crudItems.createProperties. validationResponse : ${JSON.stringify(validationResponse)} `);
    return validationResponse;
}
...
async function updateProperties(updateProperties) {
    log.debug(`Starting: crudItems.updateProperties. properties object : ${JSON.stringify(updateProperties)} `);
    ...
    if (!updateProperties.name) {
        log.debug('Update: crudItems.updateProperties. value of item name not found!');
        validationResponse.message = 'Value of item name not found!';
        validationResponse.responseCode = 456;
        return validationResponse;
    }
    ...
    // to create a fragmented item
        ...
        // Is properties currently in DB?
        log.debug('Update: crudItems.updateProperties. Is properties currently in DB? ');
        const deviceCol = await db.findOne(query);

        if (deviceCol) {
            log.debug('Update: crudItems.updateProperties : Properties found in DB.   Updating Properties');
            ...
            log.debug(`Update: crudItems.updateProperties : deviceCol : ${JSON.stringify(deviceCol)}`);
            log.debug(`Update: crudItems.updateProperties : updateOneObject : ${JSON.stringify(updateOneObject)}`);
            ...
            if (!['true', true, 1].includes(bulkWriteResults.ok)) {
                throw new Error(`crudItems.updateProperties : Failed mongo update ${JSON.stringify(updateOneObject)}`);
            }
            validationResponse.message = 'Item Properties updated!';
            validationResponse.responseCode = 200;
        } else {
            log.debug('Ending: crudItems.updateProperties : Item properties not found!  Unable to update.');
...
async function deleteProperties(deleteProperties) {
    log.debug(`Starting: crudItems.deleteProperties. properties object : ${JSON.stringify(deleteProperties)} `);
    ...
    if (!deleteProperties.name) {
        log.debug('Update: crudItems.deleteProperties. value of item name not found!');
        validationResponse.message = 'Value of item name not found!';
        validationResponse.responseCode = 456;
        return validationResponse;
    }
    ...
    // to create a fragmented item
        ...
        // Is properties currently in DB?
        log.debug('Update: crudItems.deleteProperties. Is properties currently in DB? ');
        const deviceCol = await db.findOne(query);

        if (deviceCol) {
            log.debug('Update: crudItems.deleteProperties : Properties found in DB.   Updating Properties');
            try {
                ...
                log.error(`Ending: crudItems.deleteProperties : Properties insert failed : ${JSON.stringify(err)}`);
                validationResponse.message = 'Properties delete failed.';
                validationResponse.responseCode = 400;
            }
        } else {
            log.debug('Ending: crudItems.deleteProperties : Item properties not found!  Unable to delete.');
