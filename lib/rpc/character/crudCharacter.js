...
async function createSheet(sheet) {
    ...
    log.debug(`Update: crudCharacter.createSheet. validSchema : ${validSchema} `);
    // Was the value valid?  If the validSheet value is false, then we know that we need to throw back an error
    if (!validSheet) {
    log.debug(`Update: slipedManager.buildSlipedObject : Sliped Manager Schema provided is not valid. : ${validate.errors}`)
    // Throw an error 
    log.debug('Update: crudCharacter.createSheet : validation error ');
    validationResponse.message = `Errors found : validate.errors`;
