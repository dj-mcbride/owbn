...
async function createSheet(sheet) {
    ...
    log.debug('Update: crudCharacter.createSheet. Does incoming sheet match template regulations? ');
...
async function updateSheet(updates) {
    ...
    if (!updates.player) {
        log.debug('Starting: crudCharacter.updateSheet. value of player name not found!');
    ...
    // to create a fragmented character
        ...
        if (deviceCol) {
            ...
            try {
                ...
                await db.update(updateOneObject);
