...
class Response {
    ...
    static combineResponses(mopResponses) {
        try {
            ...
            mopResponses.forEach((response) => {
                if (response.responseCode === 200) {
                    result.userMessage += response.userMessage;
                } else if (exists(response.responseCode)) {
                    ...
                    throw new Response({
                        from: response,
                        responseCode: 500,
                        message: 'Got result with no response code in Response.combineResponses',
                        step: 'Combine Responses'
                    });
                }
            ...
            throw new Response({ from: error, step: 'Combine Responses' });
        }
    }
